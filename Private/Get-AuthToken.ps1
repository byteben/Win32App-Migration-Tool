...
function Get-AuthToken {
    ...
    process {
        ...
        try {
            ...
            #Write-Log -Message ("'{0}'" -f $_.Exception.Message) -LogId $LogId -Severity 3
            #Get-ScriptEnd -ErrorMessage 'Failed to connect to get a token' -LogId $LogId
        }

        # Check if the connection is successful
        if ($token) {
            #Write-Log -Message 'Successfully connected' -LogId $LogId
            #Write-Log -Message ("Token expires on: {0}" -f $token.ExpiresOn) -LogId $LogId
            Write-Host 'Successfully connected' -ForegroundColor Green
            Write-Host ("Token expires on: {0}" -f $token.ExpiresOn) -ForegroundColor Green
            ...
            try {
                ...
                Write-Host ("Authentication header expires on: {0}" -f $token.ExpiresOn) -ForegroundColor Green
            }
            catch {
                Write-Warning -Message ("'{0}'" -f $_.Exception.Message)
