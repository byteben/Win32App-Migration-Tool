...
function New-FolderToCreate {
    ...
    Write-Log -Message 'Function: New-FolderToCreate was called' -LogId $LogId

    foreach ($folder in $Folders) {

        #Create Folders
        Write-Log -Message ("`$FolderToCreate = Join-Path -Path '{0}' -ChildPath '{1}'" -f $Root, $folder) -LogId $LogId
        ...
        If (!(Test-Path -Path $folderToCreate)) {
            ...
            try {
                ...
                Write-Log -Message ("Folder '{0}' was created succesfully" -f $folderToCreate) -LogId $LogId
                Write-Host ("Folder '{0}' created succesfully" -f $folderToCreate) -ForegroundColor Green
            }
            catch {
                Write-Log -Message ("Warning: Couldn't create '{0}' folder" -f $folderToCreate) -LogId $LogId -Severity 3
                Write-Warning -Message ("Warning: Couldn't create '{0}' folder" -f $folderToCreate)
            }
        }
        else {
            Write-Log -Message ("Information: Folder '{0}' already exsts. Skipping folder creation" -f $folderToCreate) -LogId $LogId -Severity 2
