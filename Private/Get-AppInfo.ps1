...
function Get-AppInfo {
    ...
    process {
        ...
        for ($i = 1; $i -le $applicationCount; $i++) {
            ...
            foreach ($application in $applicationName) {
                ...
                Write-Log -Message ("Invoking Get-CMApplication where Id equals '{0}' for appliction '{1}'" -f $application.Id, $application.LocalizedDisplayName) -LogId $LogId
                Write-Host ("Invoking Get-CMApplication where Id equals '{0}' for appliction '{1}'" -f $application.Id, $application.LocalizedDisplayName) -ForegroundColor Cyan
